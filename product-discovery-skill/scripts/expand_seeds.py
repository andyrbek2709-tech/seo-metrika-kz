...
для съёма спроса в Google (Keyword Planner через DataForSEO).
...
съёме (Keyword Planner вернёт 0) — поэтому «лишние» комбинации не вредят, важна полнота.
...
INTENTS: Dict[str, List[str]] = {
    ...
    # --- software-discovery: спрос на «сделать программу/приложение/расчёт» (RU) ---
    "soft_ru": [
        "программа для {q}", "приложение для {q}", "сервис для {q}", "система для {q}",
        "софт для {q}", "программа для расчета {q}", "калькулятор {q}",
        "онлайн калькулятор {q}", "автоматизация {q}", "бот для {q}",
        "crm для {q}", "программа учета {q}", "{q} онлайн", "{q} программа",
    ],
    # боль/намерение: люди ищут, чем заменить ручной труд и excel (сигнал «постройте мне это»)
    "pain_ru": [
        "как автоматизировать {q}", "нужна программа для {q}", "чем считать {q}",
        "как вести {q}", "{q} в excel", "упростить {q}", "{q} шаблон excel",
    ],
    # --- software-discovery (EN) ---
    "soft_en": [
        "software for {q}", "app for {q}", "tool for {q}", "{q} software", "{q} app",
        "{q} calculator", "{q} management software", "best software for {q}",
        "online {q} tool", "ai for {q}", "{q} saas", "automate {q}", "how to automate {q}",
    ],
